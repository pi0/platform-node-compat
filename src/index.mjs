...
<body>
  <div>
    ${Object.entries(links)
      .map(([name, url]) => /* html */ `<a href="${url}">${name}</a>`)
      .join(" | ")}
  </div>
  ...
  <div>
    Features: ${Object.entries(report.features)
      .map(([name, value]) => `${value ? "✅" : "❌"} <code>${name}</code>`)
      .join(" | ")}
  </div>
...
async function collectCompat() {
  ...
  report.globals.missing = nodeCompat.globals.globalKeys
    .filter((name) => !(name in globalThis) && !globalThis[name])
    .sort();
  ...
  for (const [id, compat] of Object.entries(nodeCompat.modules)) {
    ...
    const realExports = Object.getOwnPropertyNames(realModule)
      .filter((name) => name !== "default")
      .sort();
    const missingExports = compat.exports
      .filter((name) => !(name in realModule))
      .sort();
